...
function Messages() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (me.role === 'tutor') {
        const appointmentData = await request('/tutor/appointments');
        const tutoring = appointmentData.tutoring || [];
      ...
      <div className="messages-page">
        ...
        {user.role === 'tutor' && (
          <section className="student-rating-panel">
            <h2>Rate Students</h2>
            {uniqueStudentAppointments.length === 0 && <p>No booked students to rate yet.</p>}
